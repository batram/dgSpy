...
public static class WorkflowCatalog {
	static readonly IReadOnlyList<WorkflowTopic> topics=Array.AsReadOnly(new[] {
		...
		Topic("local_deployment","Call launch_local_host once. It installs the bundled host when needed, starts dnSpy, and waits briefly for registration. It never starts a second host beside a running one: a host already running the installed payload is adopted, and a host running a different build fails with host_already_running. Use get_local_deployment to inspect the managed install. Pass replace=true only when you mean to end that host's debugging sessions, because replacement detaches its targets and closes it.","get_local_deployment","launch_local_host"),
		Topic("remote_deployment","Ask for host_id and the Gateway address reachable from that host, then call create_remote_host_package once. Give the ZIP and SHA-256 to the user; do not transfer or run it automatically. Use get_remote_host_readiness to verify registration and connectivity, and revoke_remote_host only for deliberate cleanup.","create_remote_host_package","get_remote_host_readiness","revoke_remote_host"),
