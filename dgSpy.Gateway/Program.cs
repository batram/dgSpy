...
public static class ToolCatalog {
	...
	public static readonly object[] All = {
		...
		Tool("wait_for_event", "Wait without polling for any requested event kinds after a cursor. Returns immediately when a matching buffered event already exists; concurrent waiters are non-destructive.", new { session_id=new { type="string" }, after_event_id=new { type="integer" }, kinds=new { type="array", items=new { type="string" }, description="Optional normalized event-kind filter." }, timeout_ms=new { type="integer", minimum=1, maximum=10000, description="Default 5000. The extension clamps to 10000." } }, new[]{"session_id"}),
		Tool("get_events", "Read normalized lifecycle and debugger events without consuming them. Optional kinds filter the bounded per-session stream; truncated reports when the requested cursor is older than retained history.", new { session_id=new { type="string" }, after_event_id=new { type="integer" }, kinds=new { type="array", items=new { type="string" } } }, new[]{"session_id"}),
		Tool("get_stop_reason", "Read the latest normalized stop, or an exact retained stop event by event_id, including process, thread, breakpoint, exception, and location details when supplied by the debugger.", new { session_id=new { type="string" }, event_id=new { type="integer" } }, new[]{"session_id"}),
