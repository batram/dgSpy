...
internal static class Program {
	...
	static int Help() { Console.WriteLine("dgspy mcp|start|stop|status|doctor|configure <codex|claude|generic> [--apply]|launch-local|uninstall-local [--remove-settings]|pack-host --host-id ID --gateway-address IP [--listen-addresses IP[,IP...]|0.0.0.0] [--plaintext] [--compression none|fastest|optimal] [--output PATH]|revoke-host --host-id ID"); return 0; }
	...
	static async Task<int> PackHostAsync(string[] args) { var request=new JsonObject{{"host_id",Required(args,"--host-id")},{"gateway_address",Required(args,"--gateway-address")},{"use_tls",!args.Contains("--plaintext")}}; AddOption(request,"output_root",args,"--output"); AddOption(request,"compression",args,"--compression"); var listen=Optional(args,"--listen-addresses"); if(listen is not null) request["listen_addresses"]=new JsonArray(listen.Split(new[]{',',';',' '},StringSplitOptions.RemoveEmptyEntries|StringSplitOptions.TrimEntries).Select(value=>(JsonNode)JsonValue.Create(value)!).ToArray()); return await CallAndPrintAsync("create_remote_host_package",request); }
