...
internal sealed class WatchControlStore {
	...
	public WatchControlStore(string? path=null) { this.path=Path.GetFullPath(path??Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"HookLab","watcher-control.json")); Directory.CreateDirectory(Path.GetDirectoryName(this.path)!); AtomicFile.SweepOrphans(this.path); }
	...
	void Write(WatchControl value)=>AtomicFile.WriteText(path,JsonSerializer.Serialize(new { schemaVersion=1,paused=value.Paused,disabledProfiles=value.DisabledProfiles.OrderBy(id=>id,StringComparer.Ordinal) }));
}

/// <summary>Publish-by-rename for the small JSON state files. A writer killed between the temporary
/// write and the rename leaves <c>name.&lt;guid&gt;.tmp</c> behind; the next owner sweeps those.</summary>
internal static class AtomicFile {
	public static void WriteText(string path,string content) { var temporary=path+"."+Guid.NewGuid().ToString("N")+".tmp"; File.WriteAllText(temporary,content,new UTF8Encoding(false)); File.Move(temporary,path,true); }
	public static void SweepOrphans(string path) {
		var directory=Path.GetDirectoryName(path)!; var name=Path.GetFileName(path);
		foreach(var orphan in Directory.EnumerateFiles(directory,name+".*.tmp",SearchOption.TopDirectoryOnly)) { try { File.Delete(orphan); } catch(IOException) { } catch(UnauthorizedAccessException) { } }
	}
}
...
internal sealed class WatcherStatusStore {
	...
	string? published;
	public WatcherStatusStore(string? path=null) { this.path=Path.GetFullPath(path??Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),"HookLab","watcher-status.json")); Directory.CreateDirectory(Path.GetDirectoryName(this.path)!); AtomicFile.SweepOrphans(this.path); }
	...
	// A polling tick is the steady state, so it is recorded once when it takes over from a subscription
	// wake rather than on every tick; otherwise the timestamp alone would force a status rewrite per poll.
	public void RecordDiscoveryWake(bool subscription) { lock(gate) { var trigger=subscription?"subscription":"polling"; if(!subscription&&trigger==lastDiscoveryTrigger) return; lastDiscoveryTrigger=trigger; lastDiscoveryUtc=DateTime.UtcNow.ToString("O"); if(subscription) subscriptionWakeCount++; } }
	...
	// The runner publishes on every poll tick (10/s by default). The file only changes when its content
	// does, so an idle watcher does not rewrite it; updatedUtc records the last content change, and
	// liveness is judged by process identity (processAlive), never by this timestamp.
	void Write(WatchControl control,CatalogSnapshot catalog) {
		var value=new { schemaVersion=1,watcherProcessId=Environment.ProcessId,watcherProcessCreationUtcTicks=processCreationUtcTicks,startedUtc=startedUtc.ToString("O"),lifecycle,discoveryMode,lastDiscoveryTrigger,lastDiscoveryUtc,subscriptionWakeCount,paused=control.Paused,disabledProfiles=control.DisabledProfiles.OrderBy(id=>id,StringComparer.Ordinal),catalogGeneration=catalog.Generation,catalogError=catalog.Error,definitions=catalog.Definitions.Select(item=>new { definitionId=item.Value.Id,profileId=item.ProfileId,packageId=item.PackageId,digest=item.DefinitionSha256,notificationPolicy=item.NotificationPolicy,clrReadinessTimeoutMs=item.ClrReadinessTimeoutMs,initializationTimeoutMs=item.InitializationTimeoutMs }),lastResults=results.Values };
		var body=JsonSerializer.Serialize(value,JsonOptions); if(body==published&&File.Exists(path)) return;
		var document=JsonNode.Parse(body)!.AsObject(); document["updatedUtc"]=DateTime.UtcNow.ToString("O");
		AtomicFile.WriteText(path,document.ToJsonString(JsonOptions)); published=body;
	}
