...
internal sealed class WatcherStatusStore {
	readonly string path; readonly object gate=new(); readonly Dictionary<string,object> results=new(StringComparer.Ordinal); readonly DateTime startedUtc=DateTime.UtcNow; string lifecycle="starting",discoveryMode="polling",lastDiscoveryTrigger="startup",lastDiscoveryUtc=DateTime.UtcNow.ToString("O"); long subscriptionWakeCount;
