...
public static class OneShotInjector {
	...
	// BitConverter rather than Convert.ToHexString, which does not exist on net48. Both produce the same
	// lowercase hex for the same bytes; this digest reaches definition_sha256, so the two formulations
	// were compared against an independent SHA-256 before the change landed rather than assumed equal.
	internal static string Hex(byte[] value)=>BitConverter.ToString(value).Replace("-",String.Empty).ToLowerInvariant();
	static string Sha256(string path) { using var sha=SHA256.Create(); return Hex(sha.ComputeHash(File.ReadAllBytes(path))); }
	static Dictionary<string,string> ParseReport(string text) { var result=new Dictionary<string,string>(StringComparer.Ordinal); foreach(var line in text.Split(new[]{'\r','\n'},StringSplitOptions.RemoveEmptyEntries)) { var separator=line.IndexOf('='); if(separator>0) result[line.Substring(0,separator)]=line.Substring(separator+1); } return result; }
