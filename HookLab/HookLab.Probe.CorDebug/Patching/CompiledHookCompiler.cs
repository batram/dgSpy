...
namespace HookLab.Probe.CorDebug.Patching {
	...
	internal sealed class CompiledHook {
		internal CompiledHook(Assembly assembly, MethodInfo? prefix, MethodInfo? postfix, MethodInfo? finalizer) { Assembly = assembly; Prefix = prefix; Postfix = postfix; Finalizer = finalizer; }
		internal Assembly Assembly { get; }
		internal MethodInfo? Prefix { get; }
		internal MethodInfo? Postfix { get; }
		internal MethodInfo? Finalizer { get; }
		internal MethodInfo[] Methods => new[] { Prefix, Postfix, Finalizer }.Where(method => method != null).Cast<MethodInfo>().ToArray();
	}
	...
	internal static class CompiledHookCompiler {
		...
		internal static CompiledHook Compile(string source, MethodBase target, HookLab.Contracts.HookKind kind) {
			...
			if (kind != HookLab.Contracts.HookKind.Prefix && kind != HookLab.Contracts.HookKind.Postfix && kind != HookLab.Contracts.HookKind.Finalizer) throw new NotSupportedException("Compiled hooks currently support Prefix, Postfix, and Finalizer only.");
			using (var provider = new CSharpCodeProvider()) {
				...
				if (prefixes.Length > 1 || postfixes.Length > 1 || finalizers.Length > 1) throw new HookCompilationException(new[] { "Source may declare at most one public static method for each HookLab patch phase." });
				...
				return new CompiledHook(result.CompiledAssembly, prefixes.SingleOrDefault(), postfixes.SingleOrDefault(), finalizers.SingleOrDefault());
			}
		}
