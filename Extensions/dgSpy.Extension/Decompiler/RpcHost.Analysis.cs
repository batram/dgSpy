...
namespace dgSpy.Extension {
	sealed partial class RpcHost {
		...
		async Task<AnalysisResult> AnalyzeSymbolAsync(RpcRequest req,CancellationToken token) {
			...
			var modules=await OnDebuggerAsync(()=>ScanModules(moduleFilter),token).ConfigureAwait(false);
			return await evaluations.RunAsync(()=>{
				var edges=new List<AnalysisEdge>(); var total=0; var scannedMethods=0; var targetSymbol=Symbol(Kind(target),target.Module?.Name??"",target,target.DeclaringType?.ResolveTypeDef());
				...
				foreach(var dbg in modules) {
					var metadata=metadataService.TryGetMetadata(dbg); if(metadata is null) continue; var moduleName=metadata.Name?.ToString()??dbg.Name;
					foreach(var type in metadata.GetTypes()) {
						token.ThrowIfCancellationRequested(); var typeSymbol=Symbol("type",moduleName,type,null);
						...
						foreach(var method in type.Methods) {
							...
							scannedMethods++;
							var methodSymbol=Symbol("method",moduleName,method,type);
