	...
	static class HookLabTargetEligibility {
		...
		public static string? UnsupportedReason(int bitness,string architecture,IEnumerable<HookLabRuntimeIdentity> runtimes) {
			if(bitness!=64 || !String.Equals(architecture,"X64",StringComparison.OrdinalIgnoreCase))
				return "HookLab currently supports x64 desktop CLR v4 targets; the attached process architecture is "+architecture+" ("+bitness+"-bit).";
			var attached=(runtimes ?? throw new ArgumentNullException(nameof(runtimes))).ToArray();
			if(attached.Any(runtime=>runtime.Guid==DotNetFrameworkRuntimeGuid && runtime.Name.StartsWith("CLR v4.",StringComparison.OrdinalIgnoreCase))) return null;
			...
			return "HookLab currently supports x64 desktop CLR v4 targets; the attached process exposes "+(names.Length==0?"no managed runtime":String.Join(", ",names))+".";
		}
	}
}
