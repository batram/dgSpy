...
namespace dgSpy.Extension {
	...
	static class NativeRegisterReader {
		...
		/// <summary>The context rows, plus rip on its own. Callers need rip as a number to establish
		/// whether the context has anything to do with the managed frame they asked about; digging it back
		/// out of the anonymous rows by reflection would break the first time a row gains a field.</summary>
		public static (object[] Registers,ulong Rip) ReadX64(ulong threadId,int processId) {
			...
			try {
				...
				Add(result,"r8",context,184); Add(result,"r9",context,192); Add(result,"r10",context,200); Add(result,"r11",context,208); Add(result,"r12",context,216); Add(result,"r13",context,224); Add(result,"r14",context,232); Add(result,"r15",context,240); var rip=Add(result,"rip",context,248);
				var flags=unchecked((uint)Marshal.ReadInt32(context,68)); result.Add(new { name="rflags",value=(ulong)flags,hex="0x"+flags.ToString("X8"),bits=32 }); return (result.ToArray(),rip);
			}
			finally { Marshal.FreeHGlobal(allocation); CloseHandle(thread); }
		}
		static ulong Add(List<object> values,string name,IntPtr context,int offset) { var value=unchecked((ulong)Marshal.ReadInt64(context,offset)); values.Add(new { name,value,hex="0x"+value.ToString("X16"),bits=64 }); return value; }
