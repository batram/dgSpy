...
namespace dgSpy.Extension {
	sealed partial class RpcHost {
		...
		async Task<ObjectIdInfo> EvaluateObjectIdAsync(RpcRequest req,CancellationToken cancellationToken) { CheckSession(req); var number=(uint?)req.Arguments["object_id"] ?? throw new RpcException("invalid_arguments","object_id is required."); return await WithEvaluationAsync(req,(captured,eval)=>{ ValidateRuntime(req,captured.Frame.Runtime); var id=objectIds.GetObjectId(captured.Frame.Runtime,number) ?? throw new RpcException("object_id_not_found",$"Object ID ${number} is not active in the selected runtime."); var nodes=captured.Language.ValueNodeFactory.Create(eval,new[]{id},NodeOptions((bool?)req.Arguments["allow_func_eval"]??false)); try { return DescribeObjectId(id,"$"+number,DescribeNode(captured,nodes[0],eval,"$"+number)); } finally { manager.Close(nodes); } },cancellationToken).ConfigureAwait(false); }
		...
		async Task<EvaluatedValue[]> GetAutosAsync(RpcRequest req,CancellationToken cancellationToken) { CheckSession(req); return await WithEvaluationAsync(req,(captured,eval)=>{
			...
			try { return nodes.Select((n,i)=>DescribeNode(captured,n,eval,expressions[i])).ToArray(); } finally { manager.Close(nodes); }
		},cancellationToken).ConfigureAwait(false); }
