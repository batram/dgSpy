...
| **Planned, not yet applied** - internal breakpoint owners reach the CorDebug engine | `Extensions/dnSpy.Debugger/dnSpy.Debugger.DotNet.CorDebug/**` (`InternalsVisibleTo` plus a small internal facade over `DnDebugger.CreateBreakpoint`/`RemoveBreakpoint` and the CorDebug-thread invoke) | HookLab's atomic actions need an internal breakpoint owner that can share a location with a user breakpoint. `DnDebugger` is assembly-internal, so its `public` breakpoint members are unreachable from `dgSpy.Extension` and the stage-0 spike had to use reflection, which is rejected for production. Measured viable in `docs/local/evidence/hooklab-breakpoint-multiplex-verdict.md`; implemented by task T07. Keep the facade minimal - it exists so the extension never reaches into engine internals ad hoc. | GPLv3; see `dnSpy/dnSpy/LicenseInfo/GPLv3.txt`. |
