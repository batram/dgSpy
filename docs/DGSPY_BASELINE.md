...
| Diagnosable child-expansion failures | `Extensions/dnSpy.Debugger/dnSpy.Debugger.DotNet/Evaluation/Engine/DbgEngineValueNodeImpl.cs` | Upstream replaced every expansion failure with the localized string "Internal debugger error" against the literal expression `<expression>`, discarding the exception. Acceptable for a GUI row; over RPC it is the whole answer and names neither the member nor the reason. Now carries the exception type/message and the parent expression, and logs the full exception to debugger output. | GPLv3; see `dnSpy/dnSpy/LicenseInfo/GPLv3.txt`. |
