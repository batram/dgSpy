...
| Internal breakpoint owners reach the CorDebug engine | `dnSpy/dnSpy.Contracts.Debugger.DotNet.CorDebug/DgSpyOwnedBreakpointService.cs`; `Extensions/dnSpy.Debugger/dnSpy.Debugger.DotNet.CorDebug/Impl/DgSpyOwnedBreakpointFacade.cs` | HookLab's atomic actions need an internal breakpoint owner that can share a location with a user breakpoint. A narrow contract plus CorDebug export creates and removes only dgSpy-owned engine breakpoints and marshals those operations to the CorDebug thread. The extension never references the scanner-loaded implementation assembly, avoiding both reflection and post-package implementation rebuilds; no general engine internals are exposed. Measured viable in `docs/local/evidence/hooklab-breakpoint-multiplex-verdict.md`; productionized by task T07. | GPLv3; see `dnSpy/dnSpy/LicenseInfo/GPLv3.txt`. |
| Atomic-action lease guard | `dnSpy/dnSpy.Contracts.Debugger/DbgActionGuard.cs`; `Extensions/dnSpy.Debugger/dnSpy.Debugger/Impl/DbgManagerImpl.cs`; `Extensions/dnSpy.Debugger/dnSpy.Debugger/Impl/DbgManagerImpl.Steppers.cs`; `Extensions/dnSpy.Debugger/dnSpy.Debugger/Impl/DbgRuntimeImpl.cs`; `Extensions/dnSpy.Debugger/dnSpy.Debugger/Breakpoints/Code/DbgCodeBreakpointsServiceImpl.cs` | HookLab atomic actions must exclusively own execution, stepping, instruction-pointer and public breakpoint mutations for one process. An optional, deny-if-any MEF guard decides on the thread that mutates, not the thread that asks: every one of these entry points defers its engine call to the debugger dispatcher, so a guard consulted before the marshal decided one thing and mutated another, and a lease acquired inside that window let the queued mutation through unguarded. The caller now captures its authorization (`DbgActionGuard.CaptureAuthorization`) and the authoritative `TryGetBlock` overload runs inside the dispatched callback, immediately before the mutation; a capture taken under one owner never passes a mutation blocked by a later one. `BreakAll` keeps its check on the entry thread because it does not marshal, and `DbgCodeBreakpointsService.Add` keeps an explicitly advisory pre-check because it must answer synchronously — its authoritative check is in `AddCore`, which closes what the call created if a lease arrived in between. Zero providers preserve stock dnSpy behavior. T06's lease provider uses lock-free immutable read snapshots and reports refusals through non-modal debugger output. | GPLv3; see `dnSpy/dnSpy/LicenseInfo/GPLv3.txt`. |
...
### Never let a project build write into the packaged host root

`AppDirectories.BinDirectory` is whichever directory contains `dnSpy.Contracts.DnSpy.dll`. Both the
packaged net48 layout and the net10 publish put it under `bin\`, and dnSpy scans that directory and its
`Extensions\*` children for `*.x.dll`. Drop a second copy of that one file in the host root and
`BinDirectory` becomes the root: `bin\Extensions\` is never scanned, no extension loads, themes go
missing, and nothing is logged, because `App.xaml.cs` catches extension-load failures into a
`Debug.WriteLine` that compiles to nothing in Release.

This is not hypothetical. A `ProjectReference` from `dgSpy.Extension` to a dnSpy **implementation**
project (as opposed to the `dnSpy.Contracts.*` projects it has always referenced) made `dotnet build`
rebuild that project, whose evaluated `TargetPath` is the packaged host root, and copy its entire
transitive closure — 50 files including `dnSpy.Contracts.DnSpy.dll` — into the root. The extension then
silently stopped loading on both frameworks. Offline composition tests still passed, because they load
from a controlled directory set and cannot see a scan-root problem. It cost an evening.

The rules that follow from it:

- Reference `dnSpy.Contracts.*` projects, never an implementation project. If an extension needs a host
  service, put the contract in the relevant contracts assembly and MEF-export the implementation.
- `build-dgspy.ps1` passes `-p:BuildProjectReferences=false` and validates the layout before the build,
  after the build, and after deployment. `tools/Test-DgSpyPackagedHostLayout.ps1` asserts both
  directions — `dnSpy.Contracts.DnSpy.dll` present under `bin\`, absent from the root — plus a
  packaged-root allowlist.
- `tests/TestSupport/Fixtures/poisoned-net48-root-files.txt` is the real captured listing from the
