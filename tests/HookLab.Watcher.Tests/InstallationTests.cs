...
public sealed class InstallationTests {
	...
	public void Install_is_closed_atomic_verified_and_task_read_back() {
		...
		Assert.True(WindowsWatcherTaskScheduler.TaskCommandLine(WatcherInstaller.TaskExecutable(),arguments).Length<=261);
	}

	[Fact]
	public void Task_readback_accepts_schtasks_quote_normalization() {
		const string xml="<Task xmlns='urn:test'><Principals><Principal><LogonType>InteractiveToken</LogonType><RunLevel>HighestAvailable</RunLevel></Principal></Principals><Actions><Exec><Command>\"C:\\Windows\\powershell.exe\"</Command><Arguments>-NoProfile -File C:\\Users\\mjb\\run-installed.ps1</Arguments></Exec></Actions></Task>";
	...
	sealed class FakeTask:IWatcherTaskScheduler { public bool Registered,Removed,Match=true,FailNextReadback,FailReadbackAfterRegister; public string? Executable,Arguments; public void Register(string value,string args) { Registered=true; Executable=value; Arguments=args; if(FailReadbackAfterRegister) { FailReadbackAfterRegister=false; FailNextReadback=true; } } public void Remove()=>Removed=true; public bool Matches(string value,string args) { if(FailNextReadback) { FailNextReadback=false; return false; } return Match&&Registered&&value==Executable&&args==Arguments; } }
